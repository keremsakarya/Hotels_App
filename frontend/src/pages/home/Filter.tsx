        ...
        <form className="lg:mt-28 flex flex-col gap-4 lg:gap-10">
            <div className="flex flex-col gap-2">
                ...
                {!isLoading && (
                    <select
                        ref={whereRef}
                        onChange={(e) => handleChange("location", e.target.value)} className="border py-1 px-4 rounded-md"
            ...
            <div className="flex flex-col gap-2">
                <label className="font-bold">Konaklama Yeri İsmine Göre Ara</label>

                <input ref={inputRef} type="text" placeholder="örn:Seaside Villa" className="border py-1 px-4 rounded-md"
                    onChange={(e) => handleChange("title", e.target.value)} defaultValue={params.get("title") || ""} />
            </div>

            <div className="flex flex-col gap-2">
                <label className="font-bold">Sıralama Ölçütü</label>

                <select ref={orderRef} onChange={(e) => handleChange("order", e.target.value)} className="border py-1 px-4 rounded-md"
            ...
            <div className="flex justify-end">
                <button type="reset" onClick={handleReset} className="bg-blue-500 p-1 px-4 text-white rounded-md w-fit">Filtreleri Temizle</button>
            </div>
        </form>
    )
}
